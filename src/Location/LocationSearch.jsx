...
export const LocationSearch = ({ popup, setLocation, setPopup }) => {
  ...
  function handleSelectDist (data) {
    const { district: distName } = data;
    setQuery(distName)
    setLocation(distName)
    setPopup(false)  
    localStorage.setItem("city", distName)

    // ⚡ CHANGED: removed window.location.reload(), Navigate triggers React re-render
    Navigate(`/explore/home/${distName.toLowerCase()}`)
  }
  ...
  function handleSelect (data) {
    ...
    // ⚡ CHANGED: removed window.location.reload()
  ...
  return (
    <>
      <div className="Location-container m-auto mt-5 pt-3 w-100 p-3 m-auto bg-light" 
           ref={popupRef} onClick={(e) => e.stopPropagation()}>

        <div className="inputSearchbar d-flex align-items-center justify-content-between 
                         ps-4 pe-4 gap-2 border border-secondary-subtle p-2" >
          <div className="serachbar">
            <i className="bi bi-search" ></i>
            <input type="text" ref={inputRef} value={query} autoFocus  onChange={HandleFunction} 
                   placeholder='Search for you city'  className=' ps-3 '
                   style={{outline:'none', boxShadow:'none', border:'none'}} />
          </div>
       
          <small className={closeBtn ? "d-block h-50 btn btn-secondary p-0 ps-2 pe-2" : "d-none"} 
                 onClick={HandleClose}>x</small>
        </div>
        ...
        <div className="citiesList">
          ...
          <div className="allCity text-center">
            <p className={viewAll ? "text-center mt-2 cursor-pointer" : " mt-2 cursor-pointer text-danger text-center"} 
               onClick={()=>setViewAll(true)}>
              {viewAll ? "Other Cities" : "View All Cities" }
            </p>
              ...
              <div className="viewAll">
                <ul className='d-flex m-auto row row-cols-sm-2 row-cols-md-4 row-cols-lg-5 row-cols-xxl-10 p-0 w-100'>
                  {visibleCities.map((city,index)=>(
                    <li key={index} onClick={()=>handleSelect(city.district)}>
